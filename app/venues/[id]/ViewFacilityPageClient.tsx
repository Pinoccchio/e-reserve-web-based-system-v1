...
export default function ViewFacilityPageClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchFacilityAndStats = async () => {
      ...
      const [facilityData, statsData] = await Promise.all([
        supabase.from("facilities").select(`*, images:facility_images(id, image_url), video_url`).eq("id", id).single(),
        supabase
